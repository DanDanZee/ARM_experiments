...
volatile uint16_t systick_10ms_couter,systick_100ms_couter; //10ms和100ms计时器
  ...
  IntMasterEnable();	//处理器中断使能	
	...
	while (1)
	{
		...
		if (systick_10ms_status) //10ms定时到
		{
			...
			result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,0); //清数码管（防止拖影）
			if (cnt < 7 || k == 0) {
				result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[cnt+1+k]);				//write port 1 				
				result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,(uint8_t)(1<<(cnt+k)));		//write port 2
			} else {
		...
		if (systick_100ms_status) //100ms定时到
		{
			...
			if (++i2c_flash_cnt		>= I2C_FLASHTIME[btn_cnt]/100)
			{
				...
				//LED跑马灯
				if (cnt < 7)
					result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~(3<<cnt));	
				else
					result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~((3<<cnt)|0x1));
			}
		}
	}
