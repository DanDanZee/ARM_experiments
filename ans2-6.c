...
int main(void)
{
  ...
  IntMasterEnable();	//处理器中断使能	
	...
	while (1)
	{
		...
		//数码管显示当前时间
		result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,(uint8_t)(0));				//清数码管
		...
		if (num != 2)
		{
			result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[now/cnt]);			//write port 1 				
			result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,(uint8_t)(1<<num));	//write port 2: No.num
			now %= cnt; cnt /= 10; num++;
		} 
		else 
		{
			result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,0x40);								//write port 1: "-"				
			result = I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,(uint8_t)(0x04));		//write port 2: No.3
			num++;
		}
